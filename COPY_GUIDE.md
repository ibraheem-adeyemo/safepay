# Vaultlify — Landing Page Copy Guide
> All headline variants, CTA variants, and form copy ready to paste

---

## HEADLINE VARIANTS (Hero Section)

**Option A — Fear-based (highest emotional pull)**
> "Buy and Sell Online Without the Fear of Getting Scammed"

**Option B — Outcome-based (most benefit-driven)**
> "Your Money is Safe — Until You Get Exactly What You Paid For"

**Option C — Conversational (most relatable, talking to a real person)**
> "No More Praying After You Pay. We Hold the Money Until It's Done."

*Recommended for testing: Run A/B between A and C — A is broader, C speaks directly to the moment of pain.*

---

## SUBHEADLINE VARIANTS

**Option A**
> Vaultlify holds the buyer's money securely until the seller delivers. Nobody gets scammed. Nobody gets ghosted. You both get exactly what was promised — or your money comes back.

**Option B**
> Create a transaction link in 2 minutes. The buyer pays into a safe. The seller delivers. Both sides confirm. Funds released. Simple.

**Option C**
> We're a neutral middleman for your WhatsApp and Instagram deals. Money stays locked until both sides are happy. No more trusting strangers.

---

## CTA BUTTON VARIANTS

**Primary (above the fold)**
- "Get Early Access — It's Free →"
- "Secure My Spot Now"
- "Join the Waitlist → Start Transacting Safely"

**Secondary / Mid-page**
- "I'm In — Secure My Spot"
- "Try It First — Get Early Access"
- "Protect My Next Transaction"

**Final CTA section**
- "Secure My Free Spot Now →"
- "I Want Safe Transactions →"
- "Add Me to the Waitlist"

---

## URGENCY BANNER COPY

- "🔥 Early access closes soon — [X] free spots remaining today."
- "⚡ [X] people joined this week. [Y] spots left."
- "📢 We're launching soon — get in free before the waitlist closes."

---

## FORM FIELD LABELS & PLACEHOLDER COPY

| Field | Label | Placeholder | Error Message |
|-------|-------|-------------|---------------|
| Name | "Your first name" | "e.g. Amaka" | "We need your name to save your spot" |
| Email | "Email address" | "you@email.com" | "Email is required to send your confirmation" |
| Role | "I mostly…" | — | "Tell us if you mostly buy or sell online" |

**Role Selector Options**
- 🛒 "Buy online" (value: buyer)
- 📦 "Sell online" (value: seller)

---

## BUTTON CLICK STATES

**Idle:** "Save My Early Access Spot →"
**Loading:** Spinner + "Saving your spot…"
**Success:** Replaced by confirmation message (see below)

---

## CONFIRMATION / SUCCESS MESSAGE

**Headline:**
> "You're in, [First Name]!"

**Body:**
> We've reserved your early access spot. You'll be among the **first people** to use Vaultlify when we launch. We'll email you at **[email]** with everything you need.

**Position Badge:**
> 📌 Your position: #[number] on the waitlist

**Footer note:**
> Share with a friend who buys or sells online — they'll thank you later.

---

## PROBLEM SECTION COPY

**Section Label:** "The Problem"

**Headline:**
> "Online transactions are broken. And everyone knows it."

**Subheadline:**
> "Whether you buy or sell on WhatsApp, Instagram, Jumia, or Telegram — you've felt this fear before."

**Buyer Story Card:**
> "You find a great deal on Instagram. The seller seems legit. You pay ₦85,000. They send a tracking number. You wait… and wait. Then — nothing. They've blocked you."
> **Stat:** 1 in 3 online buyers has been scammed or knows someone who has

**Seller Story Card:**
> "A buyer messages you on WhatsApp. You ship the product. You share the waybill number as proof. Then the buyer says they'll 'pay later' — and ghosts you completely."
> **Stat:** 68% of sellers refuse bank transfers from strangers because of this exact fear

**Closing Statement:**
> "This isn't just your problem. It's everyone's problem. Billions of naira are lost every year to failed social commerce transactions. And the worst part? Most of it is completely preventable."

---

## SOLUTION SECTION COPY

**Section Label:** "The Solution"

**Headline:**
> "A neutral third party that holds the money — until the deal is done."

**Body:**
> Think of Vaultlify as a trusted middleman that both the buyer and seller agree to. The buyer's money goes into a secure hold — and only gets released when both parties are happy.
>
> No more trusting a stranger with your money. No more delivering goods on a prayer. Vaultlify makes both sides safe — at the same time.

**Bullet Points:**
- Works for physical goods, digital products & services
- Generate a transaction link in under 2 minutes
- Share it on any platform — WhatsApp, Telegram, DMs
- Full dispute resolution if something goes wrong

---

## HOW IT WORKS — STEP COPY

**Step 1 — Create a Safe Transaction Link**
> Either the buyer or seller starts a transaction. You set the amount, describe what's being bought, and get a unique protected link.

**Step 2 — Buyer Pays Into the Safe**
> The buyer sends payment — but the money doesn't go to the seller yet. It's held securely on our platform, like a digital safe deposit box.

**Step 3 — Seller Delivers the Goods**
> Now that payment is confirmed and secured, the seller delivers the item or service with confidence — knowing their money is waiting.

**Step 4 — Buyer Confirms Receipt**
> Once the buyer confirms they've received what they paid for, the funds are instantly released to the seller. Done. Clean. Safe.

---

## TRUST SECTION COPY

**Skeptical Headline:**
> "How do I know you won't disappear with my money?"

**Subheadline:**
> "We expected that question. It's a fair one. Here's exactly how we protect both sides — and why we can't run off with your funds even if we wanted to."

**Trust Points:**

1. **We never touch your money**
   Funds are held in a regulated escrow account — not our operating account. We can't spend it, we can't move it without both parties agreeing.

2. **Disputes are handled fairly**
   If there's a problem, our team reviews the evidence from both sides. We don't take sides — we follow the facts.

3. **You're in control the whole time**
   You can see the status of your transaction at any time. Nothing happens without your knowledge or approval.

4. **No account needed to get started**
   The person creating the transaction signs up. The other party just clicks the link — no registration required to pay or confirm.

**FAQ Copy:**

Q: "What if the buyer lies about not receiving the goods?"
A: We ask for proof — tracking numbers, photos, delivery confirmations. If there's a dispute, our team reviews real evidence. Liars don't win.

Q: "What if the seller delivers something broken or fake?"
A: The buyer has a review window to inspect the goods before funds are released. If something's wrong, they raise a dispute and we investigate.

Q: "How long does the money stay in hold?"
A: You and the seller agree on the inspection period upfront — typically 24 to 72 hours after delivery. It's written into the transaction.

Q: "Is this legal and regulated?"
A: Yes. We operate under standard financial regulations for payment processing and escrow services in Nigeria. Full compliance, always.

---

## FINAL CTA SECTION COPY

**Headline:**
> "The next time you buy or sell online — you shouldn't have to pray it works out."

**Body:**
> Join hundreds of buyers and sellers who are done gambling with their money on social commerce. Get in early — for free.

**Trust Micro-copy:**
- ✓ Zero cost to join
- ✓ No spam ever
- ✓ First to use the product when we launch

---

## EARLY ACCESS HOOK COPY

**Banner:**
> "Only [X] spots left in early access"

**Urgency framing options:**
- "We're limiting early access to 500 users so we can give everyone proper support."
- "Once we hit our limit, sign-ups close until launch."
- "Early users get: free transactions for the first 3 months + priority support."

---

## META / SEO COPY

**Page Title:** Vaultlify — Buy & Sell Online Without Getting Scammed

**Meta Description:** Vaultlify holds payment securely until both sides are happy. No more scams on WhatsApp or Instagram. Join the early access waitlist — free.

**OG Title:** Stop Praying After You Pay. Vaultlify Protects Every Transaction.

---

*File generated for Vaultlify Fake MVP validation test. Adjust currency symbol and legal copy per target market.*